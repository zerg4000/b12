...
- (void)performMethod:(id<QSMethod>)method completion:(QSNetworkingCompletionBlock)completion {
    ...
    NSDictionary *dataDictionary = [method.qs_requestSchema toDictionary];
    ...
    NSURL *url = [self.url URLByAppendingPathComponent:method.qs_name];
    ...
    [self.transport postData:requestData ofType:@"application/json" toURL:url ofType:@"application/json" withCompletion:^(NSError *error, NSData *data)
    {
        ...
        __unsafe_unretained Class responseSchemaClass = method.qs_responseSchemaClass;
        if(responseSchemaClass == nil) {
            completion([NSError coreErrorWithMessage:@"No class object found for schema"], nil);
            return;
        }

        QSSchema *schema = [[responseSchemaClass alloc] initWithDictionary:responseResult error:&error];
...
- (NSDictionary*)getResult:(NSDictionary*)response error:(NSError**)error {
    ...
    if(error != nil) {
        *error = [NSError serverErrorWithComprehensiveCode:errorCode message:message error:nil];
    }
